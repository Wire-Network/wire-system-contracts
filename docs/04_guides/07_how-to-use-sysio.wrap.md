---
title: How to use sysio.wrap
link_text: How to use sysio.wrap
---

# 1. Installing the sysio.wrap contract

The sysio.wrap contract needs to be installed on a privileged account to function. It is recommended to use the account `sysio.wrap`.

First, the account `sysio.wrap` needs to be created. Since it has the restricted `sysio.` prefix, only a privileged account can create this account. So this guide will use the `sysio` account to create the `sysio.wrap` account. On typical live blockchain configurations, the `sysio` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `sysio.msig` contract to help coordinate the approvals of the proposed transaction that creates the `sysio.wrap` account.

The `sysio.wrap` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`sysio`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `sysio` account needs to have enough of the core system token (the `SYS` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `sysio` account.

This guide will be using clio to carry out the process.

## 1.1 Create the sysio.wrap account

### 1.1.1 Generate the transaction to create the sysio.wrap account

The transaction to create the `sysio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clio command to propose the transaction to the sysio.msig contract.

A simple way to generate a transaction to create a new account is to use the `clio system newaccount`. However, that sub-command currently only accepts a single public key as the owner and active authority of the new account. However, the owner and active authorities of the new account should only be satisfied by the `active` permission of `sysio`. One option is to create the new account with the some newly generated key, and then later update the authorities of the new account using `clio set account permission`. This guide will take an alternative approach which atomically creates the new account in its proper configuration.

Three unsigned transactions will be generated using clio and then the actions within those transactions will be appropriately stitched together into a single transaction which will later be proposed using the sysio.msig contract.

First, generate a transaction to capture the necessary actions involved in creating a new account:
```sh
clio system newaccount -s -j -d --transfer --stake-net "1.000 SYS" --stake-cpu "1.000 SYS" --buy-ram-kbytes 50 sysio sysio.wrap SYS8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
```
```console
726964ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea305500c80000"} arg: {"code":"sysio","action":"buyrambytes","args":{"payer":"sysio","receiver":"sysio.wrap","bytes":51200}}
726967ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea3055102700000000000004535953000000001027000000000000045359530000000001"} arg: {"code":"sysio","action":"delegatebw","args":{"from":"sysio","receiver":"sysio.wrap","stake_net_quantity":"1.0000 SYS","stake_cpu_quantity":"1.0000 SYS","transfer":true}}
```
```sh
...
The actual public key used is not important since that data is only encoded into the `sysio::newaccount` action which will be replaced soon anyway.

Second, create a file (e.g. newaccount_payload.json) with the JSON payload for the real `sysio::newaccount` action. It should look like:
```sh
...
Third, generate a transaction containing the actual `sysio::newaccount` action that will be used in the final transaction:
```sh
clio push action -s -j -d sysio newaccount newaccount_payload.json -p sysio > generated_newaccount_trx.json
...
Fourth, generate a transaction containing the `sysio::setpriv` action which will make the `sysio.wrap` account privileged:
```sh
clio push action -s -j -d sysio setpriv '{"account": "sysio.wrap", "is_priv": 1}' -p sysio > generated_setpriv_trx.json
...
Next, the action JSONs of the previously generated transactions will be used to construct a unified transaction which will eventually be proposed with the sysio.msig contract. A good way to get started is to make a copy of the generated_newaccount_trx.json file (call the copied file create_wrap_account_trx.json) and edit the first three fields so it looks something like the following:
```sh
...
It will be useful to have a JSON of the active permissions of each of the active block producers for later when proposing transactions using the sysio.msig contract.
...
### 1.1.2 Propose the transaction to create the sysio.wrap account
...
#    sysio.msig <= sysio.msig::propose          {"proposer":"blkproducera","proposal_name":"createwrap","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

### 1.1.3 Review and approve the transaction to create the sysio.wrap account
...
#    sysio.msig <= sysio.msig::approve          {"proposer":"blkproducera","proposal_name":"createwrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

### 1.1.4 Execute the transaction to create the sysio.wrap account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `sysio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the that is generated by the sysio.msig contract).
...
#    sysio.msig <= sysio.msig::exec             {"proposer":"blkproducera","proposal_name":"createwrap","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the `sysio.wrap` was created:
```sh
clio get account sysio.wrap
```
```console
privileged: true
permissions:
     owner     1:    1 sysio@active,
        active     1:    1 sysio@active,
memory:
...
## 1.2 Deploy the sysio.wrap contract

### 1.2.1  Generate the transaction to deploy the sysio.wrap contract

The transaction to deploy the contract to the `sysio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clio command to propose the transaction to the sysio.msig contract.

The easy way to generate this transaction is using clio:
```sh
clio set contract -s -j -d sysio.wrap contracts/sysio.wrap/ > deploy_wrap_contract_trx.json
```
```console
Reading WAST/WASM from contracts/sysio.wrap/sysio.wrap.wasm...
...
### 1.2.2 Propose the transaction to deploy the sysio.wrap contract
...
#    sysio.msig <= sysio.msig::propose          {"proposer":"blkproducera","proposal_name":"deploywrap","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

### 1.2.3 Review and approve the transaction to deploy the sysio.wrap contract
...
#    sysio.msig <= sysio.msig::approve          {"proposer":"blkproducera","proposal_name":"deploywrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

### 1.2.4 Execute the transaction to create the sysio.wrap account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `sysio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost that is generated by the sysio.msig contract).
...
#    sysio.msig <= sysio.msig::exec             {"proposer":"blkproducera","proposal_name":"deploywrap","executer":"blkproducera"}
```

Anyone can now verify that the `sysio.wrap` contract was deployed correctly.

```sh
clio get code -a retrieved-sysio.wrap.abi sysio.wrap
```
```console
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-sysio.wrap.abi
```
```sh
sha256sum contracts/sysio.wrap/sysio.wrap.wasm
```
```console
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/sysio.wrap/sysio.wrap.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-sysio.wrap.abi, can then be compared to the original ABI of the contract (contracts/sysio.wrap/sysio.wrap.abi) to ensure they are semantically the same.

# 2. Using the sysio.wrap contract

## 2.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed sysio.wrap contract together with the sysio.msig contract to allow a greater than two-thirds supermajority of block producers of the Wire blockchain to change the owner authority of an arbitrary account. The example will use clio: in particular, the `clio multisig` command, the `clio set account permission` sub-command, and the `clio wrap exec` sub-command. However, the guide also demonstrates what to do if the `clio wrap exec` sub-command is not available.
...
Assume that none of the block producers know the private key corresponding to the public key `SYS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV` which, as can be seen above, is initially securing access to the `alice` account.
...
```sh
clio set account permission -s -j -d alice owner '{"threshold": 1, "accounts": [{"permission": {"actor": "sysio", "permission": "active"}, "weight": 1}]}' > update_alice_owner_trx.json
```
...
The next step is to generate the transaction containing the `sysio.wrap::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.
...
Then generate the template for the transaction containing the `sysio.wrap::exec` action:
```sh
clio push action -s -j -d sysio.wrap exec '{"executer": "blkproducera", "trx": ""}' > wrap_update_alice_owner_trx.json
cat wrap_update_alice_owner_trx.json
```
```json
{
...
#    sysio.msig <= sysio.msig::propose          {"proposer":"blkproducera","proposal_name":"updatealice","requested":[{"actor":"blkproducera","permi...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of nodeop/clio, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `sysio.wrap::exec` action is available in the output of the `clio multisig review` command, only the hex data is available of the payload of the inner `sysio::updateauth` action. So it is not clear what the `updateauth` will actually do.

Furthermore, even if this usability issue was fixed in nodeop/clio, there will still be cases where there is no sensible human-readable version of an action data payload within a transaction. An example of this is the proposed transaction in sub-section 2.2.3 which used the `sysio::setcode` action to set the contract code of the `sysio.wrap` account. The best thing to do in such situations is for the reviewer to compare the proposed transaction to one generated by them through a process they trust.
...
#    sysio.msig <= sysio.msig::approve          {"proposer":"blkproducera","proposal_name":"updatealice","level":{"actor":"blkproducerb","permission...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
#    sysio.msig <= sysio.msig::exec             {"proposer":"blkproducera","proposal_name":"updatealice","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```
