...
The transaction to create the `sysio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the cleos command to propose the transaction to the sysio.msig contract.
...
Three unsigned transactions will be generated using cleos and then the actions within those transactions will be appropriately stitched together into a single transaction which will later be proposed using the sysio.msig contract.

First, generate a transaction to capture the necessary actions involved in creating a new account:
```sh
cleos system newaccount -s -j -d --transfer --stake-net "1.000 EOS" --stake-cpu "1.000 EOS" --buy-ram-kbytes 50 sysio sysio.wrap EOS8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
```
```console
...
```sh
cleos push action -s -j -d sysio newaccount newaccount_payload.json -p sysio > generated_newaccount_trx.json
...
```sh
cleos push action -s -j -d sysio setpriv '{"account": "sysio.wrap", "is_priv": 1}' -p sysio > generated_setpriv_trx.json
...
The transaction to deploy the contract to the `sysio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the cleos command to propose the transaction to the sysio.msig contract.

The easy way to generate this transaction is using cleos:
```sh
cleos set contract -s -j -d sysio.wrap contracts/sysio.wrap/ > deploy_wrap_contract_trx.json
```
```console
...
```sh
cleos get code -a retrieved-sysio.wrap.abi sysio.wrap
```
```console
...
This example will demonstrate how to use the deployed sysio.wrap contract together with the sysio.msig contract to allow a greater than two-thirds supermajority of block producers of the EOS blockchain to change the owner authority of an arbitrary account. The example will use cleos: in particular, the `cleos multisig` command, the `cleos set account permission` sub-command, and the `cleos wrap exec` sub-command. However, the guide also demonstrates what to do if the `cleos wrap exec` sub-command is not available.
...
```sh
cleos set account permission -s -j -d alice owner '{"threshold": 1, "accounts": [{"permission": {"actor": "sysio", "permission": "active"}, "weight": 1}]}' > update_alice_owner_trx.json
```
...
```sh
cleos push action -s -j -d sysio.wrap exec '{"executer": "blkproducera", "trx": ""}' > wrap_update_alice_owner_trx.json
cat wrap_update_alice_owner_trx.json
```
```json
{
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of nodeos/cleos, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `sysio.wrap::exec` action is available in the output of the `cleos multisig review` command, only the hex data is available of the payload of the inner `sysio::updateauth` action. So it is not clear what the `updateauth` will actually do.
