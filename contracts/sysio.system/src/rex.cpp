...
#include <sysio.system/rex.results.hpp>
...
namespace sysiosystem {
         ...
         for ( uint16_t i = 0; i < max; ++i ) {
            ...
            if ( bitr != _rexbalance.end() ) { // should always be true
               ...
               if ( result.success ) {
                  ...
                  rex_results::orderresult_action order_act( rex_account, std::vector<sysio::permission_level>{ } );
   ...
   int64_t system_contract::rent_rex( T& table, const name& from, const name& receiver, const asset& payment, const asset& fund )
   {
      ...
      table.emplace( from, [&]( auto& c ) {
         ...
         c.expiration   = current_time_point() + sysio::days(30);
         c.loan_num     = pool->loan_num;
      });

      rex_results::rentresult_action rentresult_act{ rex_account, std::vector<sysio::permission_level>{ } };
   ...
   {
      /**
       * If CORE_SYMBOL is (SYS,4), maximum supply is 10^10 tokens (10 billion tokens), i.e., maximum amount
