...
struct powerup_tester : sysio_system_tester {

   powerup_tester() { create_accounts_with_resources({ "sysio.reserv"_n }); }
   ...
   void check_powerup(const name& payer, const name& receiver, uint32_t days, int64_t net_frac, int64_t cpu_frac,
      ...
      auto before_reserve  = get_account_info("sysio.reserv"_n);
      ...
      auto after_reserve  = get_account_info("sysio.reserv"_n);
...
BOOST_AUTO_TEST_SUITE(sysio_system_powerup_tests)

BOOST_FIXTURE_TEST_CASE(config_tests, powerup_tester) try {
   BOOST_REQUIRE_EQUAL("missing authority of sysio",
...
BOOST_FIXTURE_TEST_CASE(weight_tests, powerup_tester) try {
   ...
   auto check_weight = [&] {
      ...
      BOOST_REQUIRE(near(           //
            ...
            int64_t(state.net.assumed_stake_weight * sysio::chain::int128_t(powerup_frac) /
